...
// @Description Активные события, которые начнутся в ближайшие 72 часа и уже заполнены на 50%+
// @Tags Discover
// @Produce json
...
func (h *DiscoverHandler) GetHotEvents(c *gin.Context) {
	...
	result := make([]HotEventResponse, 0, len(events))
...
func (h *DiscoverHandler) GetRecommendedEvents(c *gin.Context) {
	...
	// 3. Получить активные будущие события
	now := time.Now().UTC()
	var events []models.Event
	database.DB.
		Preload("Participants").
