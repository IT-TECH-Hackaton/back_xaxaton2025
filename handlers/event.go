...
func (h *EventHandler) GetEvents(c *gin.Context) {
	...
	if sortBy == "distance" && latStr != "" && lonStr != "" {
		...
		// События с продвижением «Топ» — в начале списка
		topPromoOrder := "(CASE WHEN id IN (SELECT event_id FROM event_promotions ep JOIN promotion_packages pp ON pp.id = ep.package_id WHERE ep.status = 'Активно' AND ep.end_date > NOW() AND ep.deleted_at IS NULL AND pp.name = 'Топ') THEN 0 ELSE 1 END)"
		if sortOrder == "DESC" {
			orderBy = topPromoOrder + " ASC, start_date DESC"
		} else {
			orderBy = topPromoOrder + " ASC, start_date ASC"
		}
	}
