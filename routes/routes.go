...
func SetupRoutes() *gin.Engine {
	...
	api := r.Group("/api")
	{
		...
		{
			payments.POST("", middleware.AuthMiddleware(), middleware.RateLimitMiddleware("10-H"), paymentHandler.CreatePayment)
			payments.POST("/webhook/yookassa", middleware.RateLimitMiddleware("100-H"), paymentHandler.YooKassaWebhook)
		}
		...
		subscription.Use(middleware.AuthMiddleware())
		{
			...
			subscription.POST("/upgrade", middleware.RateLimitMiddleware("5-H"), subscriptionHandler.UpgradeSubscription)
		}

		promotionHandler := handlers.NewPromotionHandler()
		promotion := api.Group("/promotion")
		{
			promotion.GET("/packages", promotionHandler.GetPackages)
